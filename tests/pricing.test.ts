...
import { MODEL_IDS } from "@/lib/claude/models";
...
describe("estimateCostUsd", () => {
  it("prices plain input and output at the model's rates", () => {
    ...
    const cost = estimateCostUsd(MODEL_IDS.haiku, {
      ...noUsage,
  ...
  it("charges cache writes at 1.25x and cache reads at 0.1x input", () => {
    ...
    const cost = estimateCostUsd(MODEL_IDS.sonnet, {
      ...noUsage,
  ...
  it("makes a cached wardrobe catalog far cheaper than resending it", () => {
    ...
    const fresh = estimateCostUsd(MODEL_IDS.sonnet, {
      ...noUsage,
      inputTokens: catalogTokens,
    });
    const cached = estimateCostUsd(MODEL_IDS.sonnet, {
      ...noUsage,
  ...
  it("returns zero for an unknown model rather than inventing a rate", () => {
    expect(
